...
class Position:
    ...
    def next_timeframe(self, exchange: ccxt, force_timeframe: str=None, force_size_factor: float=None, timeframes: list=None):
        ...
        self.current_base_size = self.base_order_size + self.current_time_level*4*self.base_order_size if not force_size_factor else force_size_factor
        self.fetch_current_market_state(exchange)

    def parse_current_review(self, market_states:dict, timeframe: str=None):
    ...
    def create_orders_by_side(self, exchange: ccxt, side: str, target_factor:float=1, place_orders: bool=False, logger=None):
        if len(self.current_market_state) == 0:
            self.fetch_current_market_state(exchange)
            self.update_current_wave_pairs()
