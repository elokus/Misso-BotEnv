...
from Misso.updater.websocket_manager import WebsocketManager
...
class FtxWebsocketClient(WebsocketManager):
    ...
    def __init__(self, subaccount) -> None:
        ...
        config = mh.get_exchange_config_for_subbaccount(subaccount)
        self._api_key = config["api_key"]
        self._api_secret = config["api_secret"]
        self._subaccount_name = subaccount
    ...
    def get_trades(self, market: str) -> List[Dict]:
        if not "-" in market:
            market = mh.reunify_symbol(market)
